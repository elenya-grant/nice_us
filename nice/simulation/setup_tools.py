...
from nice.tools.file_tools import check_create_folder, load_yaml
...
def make_tech_config(tech_names, existing_case, add_on_case, existing_pv_type="fixed"):
    ...
    remaining_techs = set(tech_names) - set(tech_config)
    for remaining_tech in remaining_techs:
        tech_config = add_tech_to_config(
            tech_config, remaining_tech, f"{remaining_tech}.yaml"
        )
    ...
    if bool(not_found_techs):
        msg = f"Missing techs: {sorted(not_found_techs)} in tech_config"
        raise ValueError(msg)
...
def make_h2i_input_files(existing_plant_case, add_on_case, **existing_plant_kwargs):
    ...
    case_folder = (
        LIBRARY_DIR / "h2i" / f"existing_{existing_plant_case}_add_{add_on_case}"
    )
    ...
    if existing_plant_case == "solar" and "array_type" not in existing_plant_kwargs:
        msg = "Please provide array_type in kwargs if existing plant is solar"
        raise ValueError(msg)
        ...
        tech_config = make_tech_config(
            ...
            existing_plant_case,
            add_on_case,
            existing_pv_type=existing_plant_kwargs["array_type"],
        )
    ...
    if existing_plant_case == "thermal" and "n_facilities" not in existing_plant_kwargs:
        msg = "Please provide n_facilities in kwargs if existing plant is thermal"
        raise ValueError(msg)
    ...
    if existing_plant_case == "thermal":
        ...
        doe_csv_filename = (
            f"existing_{pv_type}_{existing_plant_case}_add_{add_on_case}_doe.csv"
        )
        driver_config_filename = f"driver_config_{pv_type}.yaml"
        recorder_filename = (
            f"existing_{pv_type}_{existing_plant_case}_add_{add_on_case}.sql"
        )
    else:
    ...
    driver_config["driver"]["parameter_sweep"]["filename"] = (
        case_folder / doe_csv_filename
    )
